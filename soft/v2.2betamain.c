 ...
 *
 *	Ver 2.2 beta
 *	-added amperometer(for calibration)
 */
...
max_v_dcr = 1250;//default: 650 for short protection- voltage decrease at max battery current
//@up
...
amp15 = 30000;
t_warm2 = 50;
cell_ind = 40; // voltage decrease(mV) / current(A)
...
while(!(PINC & fire) && (PINC & minus) && (PINC & plus))
	...
	while(v_dcr <= max_v_dcr && l_v >= min_l_v && !(PINC & fire) && (mode == 0) && (PINC & minus) && (PINC & plus))
		...
		if ( amperage >= amp6 ) PORTD = (1<<warm) | (1<<ind1) | (1<<ind2) | (1<<ind3) |(0<<ind4) | (0<<bl);
