...
v_check = 350; //(us) 2000 single, 350 for serial(for 100uF cap) checking load voltages- time for discharge cell capacitor
...
max_v_dcr = 950;//(on one cell)default: 650 for short protection- voltage decrease at max battery current
//@up
...
min_v_dcr = 50; //10- for single, 50 for serial//(on one cell)no atomizer- min voltage decrease// 2 not tested yet
...
int t_warm2, cell_ind, amp1, amp2, amp3, amp4, amp5, amp6, amp7, amp8, amp9, amp10, amp11, amp12, amp13, amp14, amp15;
amp1 = 2000;
amp2 = 4000;
amp3 = 6000;
amp4 = 8000;
...
amp15 = 30000;
t_warm2 = 240;
cell_ind = 35; // voltage decrease(mV, on one cell) / current(A)
