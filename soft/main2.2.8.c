...
//times
int t_blink, t_splash, t_press, t_pause, v_check, t_adj_s, t_adj_f, v_display, t_unlock, tph, tph1, tph2, tph3, tph4;
...
//battery indicator steps(for one cell)
vs1 = 3600;
vs2 = 3700;
vs3 = 3800;
vs4 = 4000;
...
int t_warm2, cal, cell_ind, cell_ind_rst, max_curr, amp1, amp2, amp3, amp4, amp5, amp6, amp7, amp8, amp9, amp10, amp11, amp12, amp13, amp14, amp15;
amp1 = 2000;
amp2 = 4000;
amp3 = 6000;
amp4 = 8000;
...
t_warm2 = 240;
cal = 0;
cell_ind = 35; // voltage decrease(mV, on one cell) / current(A) - for amperometer - reading from eeprom
max_curr = 20;//in amperes
max_v_dcr = cell_ind * max_curr;
...
//frequently changed
cell_ind_rst = 35; // default value for cell indicator- for restore from cal
//max_v_dcr_rst = 700; //(on one cell)default: 650 for short protection- voltage decrease at max battery current
min_v_dcr = 10; //10- for single, 50 for serial//(on one cell)no atomizer- min voltage decrease
v_check = 2000; //(us) 2000 single, 350 for serial(for 100uF cap) checking load voltages- time for align cell capacitor
...
cell_ind = eeprom_read_byte(&c); // cell indicator
...
//count maximum voltage decrease
max_v_dcr = cell_ind * max_curr;
...
// amp calibrate
if (!(PINC & fire) && (PINC & minus) && !(PINC & plus) && (cal == 0))
	{
	_delay_ms(t_unlock);
	if (!(PINC & fire) && (PINC & minus) && !(PINC & plus) && (cal == 0))
		{
...
//plus
	if ((PINC & fire) && (PINC & minus) && !(PINC & plus) && (cell_ind < 65))
		{
		PORTD = (0<<ind1) | (1<<ind2) | (1<<ind3) |(0<<ind4) | (0<<buz) | (1<<bl);
		cell_ind = cell_ind + 1;
		max_v_dcr = cell_ind * max_curr;
		_delay_ms(t_press);
		}
...
//minus
	if ((PINC & fire) && !(PINC & minus) && (PINC & plus) && (cell_ind > 15))
		{
		PORTD = (1<<ind1) | (1<<ind2) | (0<<ind3) |(0<<ind4) | (0<<buz) | (1<<bl);
		cell_ind = cell_ind - 1;
		max_v_dcr = cell_ind * max_curr;
...
while(!(PINC & fire) && (PINC & minus) && (PINC & plus))
	...
	while(v_dcr < max_v_dcr && l_v > min_l_v && !(PINC & fire) && (mode == 0) && (PINC & minus) && (PINC & plus))
		...
		//amperometer
		_delay_us(v_check);
		max_amp = v_dcr / cell_ind;
		multiplier = duty * 4;
		amperage = max_amp * multiplier;

		if ( amperage < amp1 ) PORTD = (1<<warm) | (0<<ind1) | (0<<ind2) | (0<<ind3) |(0<<ind4) | (0<<bl);
		if ( amperage >= amp1 ) PORTD = (1<<warm) | (1<<ind1) | (0<<ind2) | (0<<ind3) |(0<<ind4) | (0<<bl);
		if ( amperage >= amp2 ) PORTD = (1<<warm) | (0<<ind1) | (1<<ind2) | (0<<ind3) |(0<<ind4) | (0<<bl);
		if ( amperage >= amp3 ) PORTD = (1<<warm) | (1<<ind1) | (1<<ind2) | (0<<ind3) |(0<<ind4) | (0<<bl);
		if ( amperage >= amp4 ) PORTD = (1<<warm) | (0<<ind1) | (0<<ind2) | (1<<ind3) |(0<<ind4) | (0<<bl);
		if ( amperage >= amp5 ) PORTD = (1<<warm) | (1<<ind1) | (0<<ind2) | (1<<ind3) |(0<<ind4) | (0<<bl);
		if ( amperage >= amp6 ) PORTD = (1<<warm) | (0<<ind1) | (1<<ind2) | (1<<ind3) |(0<<ind4) | (0<<bl);
		if ( amperage >= amp7 ) PORTD = (1<<warm) | (1<<ind1) | (1<<ind2) | (1<<ind3) |(0<<ind4) | (0<<bl);
		if ( amperage >= amp8 ) PORTD = (1<<warm) | (0<<ind1) | (0<<ind2) | (0<<ind3) |(1<<ind4) | (0<<bl);
		if ( amperage >= amp9 ) PORTD = (1<<warm) | (1<<ind1) | (0<<ind2) | (0<<ind3) |(1<<ind4) | (0<<bl);
		if ( amperage >= amp10 ) PORTD = (1<<warm) | (0<<ind1) | (1<<ind2) | (0<<ind3) |(1<<ind4) | (0<<bl);
		if ( amperage >= amp11 ) PORTD = (1<<warm) | (1<<ind1) | (1<<ind2) | (0<<ind3) |(1<<ind4) | (0<<bl);
		if ( amperage >= amp12 ) PORTD = (1<<warm) | (0<<ind1) | (0<<ind2) | (1<<ind3) |(1<<ind4) | (0<<bl);
		if ( amperage >= amp13 ) PORTD = (1<<warm) | (1<<ind1) | (0<<ind2) | (1<<ind3) |(1<<ind4) | (0<<bl);
		if ( amperage >= amp14 ) PORTD = (1<<warm) | (1<<ind1) | (1<<ind2) | (1<<ind3) |(1<<ind4) | (0<<bl);
		if ( amperage >= amp15 ) PORTD = (1<<warm) | (1<<ind1) | (1<<ind2) | (1<<ind3) |(1<<ind4) | (0<<bl);
